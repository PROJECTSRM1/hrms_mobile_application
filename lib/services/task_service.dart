...
class TaskService {
 ...
 static Future<List<TaskHistory>> getTaskHistory() async {
  ...
  if (response.statusCode == 200) {
    ...
    return data.map((e) => TaskHistory.fromJson(e)).toList();
  }
 ...
 static Future<List<Map<String, dynamic>>> getAssignedTasks() async {
  ...
  return tasks.where((t) => t.empId != null).map((t) {
    ...
    final mgr = managers.firstWhere(
      (m) => m['id'] == t.reportingManagerId,
      orElse: () => {'name': 'N/A'},
    );
    ...
    return {
      ...
      'projectName': proj['name'],      // ✅ from projects
      'reportingManager': mgr['name'],  // ✅ from managers
      'taskTitle': t.title,
      'taskDescription': t.description,
      'effortsInDays': t.effortsInDays ?? 0,
      'taskStatus': 'Active', // or map from statusId
      'dueDate': t.dueDate ?? '',
    };
  }).toList();
}
